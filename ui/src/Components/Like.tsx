...
const Like = ({ postId, initialLikes = 0, initialLikeStatus = false, onLikeChange }: LikeButtonProps) => {
    ...
    const handleLikeToggle = async () => {
        try {
            ...
            if (response.ok) {
                ...
                setIsLiked(!isLiked);
                onLikeChange?.(data.likes, !isLiked);
            } else {
                console.error(`Failed to ${isLiked ? "unlike" : "like"} post`);
            }
        } catch (e) {
            console.error(`Error ${isLiked ? "unliking" : "liking"} post`, e);
        }
    };
